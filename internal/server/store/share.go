...
func (s *Store) CreateShare(clientUID, shareName, kind, localPath string, localPort, processPID int, processExe string) (*Share, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	now := time.Now().Unix()
	res, err := s.db.Exec(`INSERT INTO shares (client_uid, share_name, kind, local_path, local_port, process_pid, process_exe, process_alive, status, online_at)
		VALUES (?,?,?,?,?,?,?,1,'active',?)`,
		clientUID, shareName, kind, localPath, localPort, processPID, processExe, now)
	if err != nil {
		return nil, err
	}
...
func (s *Store) GetShare(clientUID, shareName string) (*Share, error) {
	...
	err := s.db.QueryRow(`SELECT id, client_uid, share_name, kind, COALESCE(local_path,''), COALESCE(local_port,0),
		COALESCE(process_pid,0), COALESCE(process_exe,''), process_alive, status, COALESCE(online_at,0), COALESCE(offline_at,0), COALESCE(closed_at,0)
		...
		Scan(&sh.ID, &sh.ClientUID, &sh.ShareName, &sh.Kind, &sh.LocalPath, &sh.LocalPort,
			&sh.ProcessPID, &sh.ProcessExe, &processAlive, &sh.Status, &sh.OnlineAt, &sh.OfflineAt, &sh.ClosedAt)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ListSharesByClient(clientUID string) ([]Share, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&sh.ID, &sh.ClientUID, &sh.ShareName, &sh.Kind, &sh.LocalPath, &sh.LocalPort,
			&sh.ProcessPID, &sh.ProcessExe, &processAlive, &sh.Status, &sh.OnlineAt, &sh.OfflineAt, &sh.ClosedAt); err != nil {
			return nil, err
		}
		sh.ProcessAlive = processAlive == 1
...
func (s *Store) ResolveShareName(clientUID, sourceKey, hintName string) (string, error) {
	...
	for i := 2; ; i++ {
		var count int
		s.db.QueryRow("SELECT COUNT(*) FROM shares WHERE client_uid=? AND share_name=?", clientUID, name).Scan(&count)
		if count == 0 {
			break
		}
